...
8. 与 OOP 冲突吗
  * 值得好好思考
  * 利用 setXXX 和 dispatch 的稳定性，在 oop 中进行反向操作（错的离谱），或者 oop 对象发射事件
...
* 暴露方法而不是属性的另一个优势：如果属性发生更改（重新赋值），通过方法总是能获取最新值，属性就做不到了
...
* React Hooks
  * 处理请求取消
  * usePersistFn 或 useMemoizedFn
  * useDebounceFn
  * useThrottleFn
  * useRefState
  * useSetState：类似于 this.setState
  * useGetState：额外提供一个稳定的 getState 参数，常用与不方便将 state 传入 deps 的情况，比 useRefState 更合适，毕竟使用 `.current` 在我看来是一件非常恶心的事情
  * useSafeState：组件卸载后，setState 不再执行
  * useUpdateEffect
  * usePrevious
  * useMounted
  * useUnmounted
  * useHistoryStack：更酷的是实现一个通用的 useHistoryTravel
  * useLocalStorage：更合适的命名 useLocalStorageState，以后还可以有 useCookieState、ueSessionStorageState
  * useWindowClientSize
  * useScrollIntoView
  * useMap/useSet：对于可变异对象的封装，本质上就是对造成变异方法进行封装，先创建新对象，在调用变异方法
  * useControllableValue：解决组件受控问题，到底是内部 state 管理，还是管理 props，amazing，非常 trick 的实现方式
  * useCreation：useRef 或 useMemo 的替代品，因为 useMemo 在未来不保证一定不会被重计算，因为 render 阶段可能会重复执行，而对于 useRef 每次重渲染，都会重新执行，对接 OOP 对象必备
...
* Toggle
* Tool
* 组件派生属性在 hooks 中如何实现，下面 class 组件实现

...
基础定义
* end point：只有一个相邻点
* branch point：有三个或更多相邻点

路径相关定义
* walk：一系列顶点，相邻顶点之间有边。
* trail：一个不经过重复的边的 walk
* path：一个不经过重复的点的 walk（因此也不经过重复的边，是一个 trail）
* closed walk：一个 walk 的起点和终点相同
* cycle：除起点和终点外不经过重复的点的 trail
* filament：不经过重复点，且除了首尾外，每个点只有两个相邻边，首尾是 end point 或 branch point。此外，顶点的折线不能是任何 cycle 的子序列。英语描述还挺形象的，理解成细丝

针对 planar graphs 欧拉公式：V - E + F = 2

核心问题
* 对边进行分类，以及确定寻找下一个边的逻辑
  * 根据每条边的端点，构造双向边图
  * 尽可能顺时针寻找
  * 尽可能逆时针寻找
* 提取 cycle 和移除 cycle edge
  * 根据寻路规则得到一个 walk
  * 删除可删的 edge，然后继续进行循环检测
    * 因为我们总是从左下角开始遍历，因此第一条边应该只属于一个空间，因此第一条边一定是可以删除的
    * 可以通过继续逆时针遍历来移除额外的边。当我们访问只有两个相邻顶点的顶点时，一旦我们到达一个有三个或更多相邻顶点的顶点，我们就不能再移除任何边了
    * filament 移除
* 处理循环嵌套和 close walk
  * 嵌套路径位于为开始节点，则掐除即可
  * 嵌套路径位于开始节点，会导致没有找到，此时需要特殊处理
  * 处理 reduced close work 没有 cycle 的情况，掐除子 cycle 后不足以构成 cycle
* 确保合法的输入
  * 每一个 edge 由 Vi0 和 Vi1 组成，其中 i0 !== i1
  * 不能允许 (i0, i1) 和 (i1, i0) 同时作为 edge 存在
  * 不能允许 overlap 的情况存在

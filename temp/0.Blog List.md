...
* Suspense 完整支持
  ...
  * 适合与 transition API 结合使用，如果你在 transition 过程中挂起（suspend）了，React 将会阻止已经可见的内容被 fallback 替换，React 会延迟渲染知道数据加载完毕，避免糟糕的加载状态
...
* startTransition API
  ...
  * 非紧急更新，可以使用 startTransition 包裹，ui 状态从 A 状态切换到 B 状态。被 startTransition 回调包裹的 setState 触发的渲染 被标记为不紧急的渲染，这些渲染可能被其他紧急渲染所抢占
* SuspenseList
...
* 如何处理连锁反应：一个状态的修改会影响其他状态，如果使用 effect 将是噩梦的开始，而是应该是运行期计算或事件处理
...
* 很多时候，你不希望 effect 运行多次，但却需要一些状态，你不应该通过禁用 linter 来解决
...
TODO: 研究下 React-Router 是如何提供数据获取能力的
...
optimistic
```js
import type { Plugin } from 'ahooks/lib/useRequest/src/types';

/**
 * @description useRequest 不同于 react/query 和 rtk/query，没有一个统一的上下文，每个 request 是独立的，对其他 request 无感知
 * @param fetchInstance
 * @param options
 * @returns
 */
const useOptimisticPlugin: Plugin<any, any[]> = (fetchInstance, options) => {
  return {};
};

export default useOptimisticPlugin;

```

设置时间间隔
```js
this.setTimeDeviation(new Date(res.headers('date')) - localTime)
```
...
根据 endpoints 断定 isLoading 字段，并不考虑到参数不同

...
### area 标签
area 标签和 a 标签非常相似，不同的是，它不是问本型超链接，而是区域型的链接。

area 标签支持的 rel 与 a 完全一样。

area 是整个 html 规则中唯一支持非举行热区的标签，它的 shape 属性支持三种类型
...
* 当需要块级盒：插入块级格式化上下文
* 当遇到行内级盒或者文字：首先尝试插入一个行内级格式化上下文，如果拍不下，那么创建一个行盒，先将行盒排版（行盒是块级，所以是第一种情况），行盒会创建一个行内级格式化上下文
...
总得来说：正常流u局主要是使用 inline-block 来作为内容的容器，利用块级格式化上下文的纵向排布和行内级格式化上下文的横向排布来完成布局的，我们需要根据需求的横向和纵向排布要求，来选择元素的 display 属性。
...
不过这里的 Rules 可就没那么简单了，它可能是 CSS 的 at-rule，也可能是普通的样式规则，不同的 rule 类型，具有不同的属性。多数 at-rule 对应着一个 rule 类型
* CSSStyleRule
* CSSCharsetRule
* CSSImportRule
* CSSMediaRule
...
> 一些浏览器处于安全考虑没有实现，也不是用于移动端浏览器，这部分即可。
...
元素滚动 API，在 Element 类为了支持滚动，加入了一下 API
...
元素的布局信息
* getClientRects：返回一个列表，里面包含元素对应的每一个盒所占据的客户端矩形区域，可以用 x,y,width,height 来获取它的位置和hi存

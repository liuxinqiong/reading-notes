...
不要过于追求将类代码量整到最小：一个类应该有一定的体型，不胖不瘦才是最好，胖了就要分拆，瘦了可能就需要合并。
...
面向对象编程风格是一种自底向上的思考方式，不是按照流程来分解任务，而是将任务翻译成一个一个小的模块（类），设计类与类之间的交互，最后按照流程将类组装起来，完成整个任务。因此养成**自底向上**思考方式很关键。
...
* **高内聚用来指导类本身的设计，松耦合用来知道类与类之间依赖关系的设计**。两者并非完全独立不相干。高内聚有助于松耦合、松耦合又需要高内聚
...
* 尽管不根据不同的类型创建不同的对象，但是**单个对象本身的创建过程比较复杂，比如需要组合其他类对象，做各种初始化操作，也可以考虑使用工厂模式**
